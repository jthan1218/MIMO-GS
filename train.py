...
def evaluate_and_save_random_test_samples(
    ...
    for output_index in range(num_samples):
        ...
        # Display only: use a shared 50 dB range below both panels' maximum.
        dynamic_range_db = 50.0
        peak_power = max(float(ground_truth_map_numpy.max()), float(predicted_map_numpy.max()))
        if peak_power <= 0.0:
            peak_power = 1.0  # Finite display range when both maps are zero.
        power_floor = peak_power * 10.0 ** (-dynamic_range_db / 10.0)
        gt_db = 10.0 * np.log10(np.maximum(ground_truth_map_numpy, power_floor))
        pred_db = 10.0 * np.log10(np.maximum(predicted_map_numpy, power_floor))
        vmax_db = 10.0 * np.log10(peak_power)
        vmin_db = vmax_db - dynamic_range_db
        ...
        ground_truth_image = axes[0].imshow(gt_db,aspect="equal",interpolation="nearest",vmin=vmin_db,vmax=vmax_db)
        ...
        ground_truth_colorbar.set_label("Power (dB)")
        ground_truth_colorbar.ax.yaxis.set_major_formatter(FormatStrFormatter("%.1f"))
        ground_truth_colorbar.update_ticks()

        predicted_image = axes[1].imshow(pred_db,aspect="equal",interpolation="nearest",vmin=vmin_db,vmax=vmax_db)
        ...
        predicted_colorbar.set_label("Power (dB)")
...
def training(
    ...
    enable_densification = int(getattr(opt_params, "enable_densification", 0))
    if enable_densification not in (0, 1):
        raise ValueError("enable_densification must be 0 or 1")
    if enable_densification:
        densify_from = int(getattr(opt_params, "densify_from_iter", 1000))
        densify_until = int(getattr(opt_params, "densify_until_iter", 15000))
        densify_interval = int(getattr(opt_params, "densification_interval", 1000))
        densify_grad = float(getattr(opt_params, "densify_grad_threshold", 5e-4))
        if not (0 <= densify_from < densify_until and densify_interval > 0):
            raise ValueError("Require 0 <= densify_from_iter < densify_until_iter and interval > 0")
        if not math.isfinite(densify_grad) or densify_grad < 0:
            raise ValueError("densify_grad_threshold must be finite and nonnegative")
    ...
    for epoch in range(scene.num_epochs):
        for magnitude, rx_pos in scene.train_iter:
            ...
            if enable_densification and iteration > densify_from:
                with torch.no_grad():
                    gaussians.begin_densification(iteration)
                    if iteration >= densify_until:
                        # Runs every subsequent step, not just on event steps.
                        gaussians.freeze_population(iteration)
                    elif iteration % densify_interval == 0:
                        gaussians.densify_and_prune(
                            max_grad=densify_grad,
                            min_opacity=1e-3,
    ...
    if enable_densification:
        state = gaussians.densification_state or {}
        print(f"[Population] final_N={gaussians.get_xyz.shape[0]} frozen_N={state.get('frozen_n')}")
