...
class GaussianModel:
    ...
    def capture(self):
        return (
            ...
            # Population bounds/latches are global metadata, not primitive rows.
            None if self.densification_state is None else dict(self.densification_state),
        )
    ...
    def restore(self, model_args, training_args):
        ...
        state = model_args[17] if len(model_args) > 17 else None
        state_keys = {"n0", "lower", "upper", "growth_stopped", "pruning_stopped", "frozen_n"}
        self.densification_state = (
            dict(state) if isinstance(state, dict) and state_keys <= state.keys() else None
        )
    ...
    def cat_tensors_to_optimizer(
        self, tensors_dict: Dict[str, torch.Tensor], keep_mask: Optional[torch.Tensor] = None
    ):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            ...
            # Replace split parents and append children in one operation.
            retained = old_param if keep_mask is None else old_param[keep_mask]
            new_tensor = torch.cat((retained, extension_tensor), dim=0)
    ...
    def densification_postfix(
        ...
        optimizable_tensors = self.cat_tensors_to_optimizer(tensors_dict, keep_mask=keep_mask)
    ...
    def densify_and_clone(
        ...
        selected_pts_mask: Optional[torch.Tensor] = None,
    ):
    ...
    def densify_and_split(
        ...
        new_rotation = self._rotation[selected_pts_mask].repeat(n_splits, 1)
        # Beam weights are normalized: divide the parent's activated opacity
        # among its children instead of duplicating its contribution weight.
        new_opacity = self.inverse_opacity_activation(
            self.get_opacity[selected_pts_mask].repeat(n_splits, 1) / n_splits
        )
        ...
        if self.tie_covariance:
            ...
            new_rotation_tx = self._rotation_tx[selected_pts_mask].repeat(n_splits, 1)

        self.densification_postfix(
            new_xyz, new_opacity, new_scaling, new_rotation, new_xyz_tx,
            new_scaling_tx, new_rotation_tx, keep_mask=~selected_pts_mask,
        )

    def begin_densification(self, iteration: int):
        if self.densification_state is None:
            n0 = self.get_xyz.shape[0]
            if n0 < 1:
                raise ValueError("Densification requires a nonempty population")
            self.densification_state = {
                "n0": n0, "lower": (n0 + 1) // 2, "upper": (3 * n0) // 2,
                "growth_stopped": False, "pruning_stopped": False, "frozen_n": None,
            }
            print(f"[PopulationWindow] iter={iteration} N0={n0} "
                  f"lower={self.densification_state['lower']} upper={self.densification_state['upper']}")
            self._check_population_bounds(iteration)

    def _check_population_bounds(self, iteration: int):
        state = self.densification_state
        n = self.get_xyz.shape[0]
        assert state["lower"] <= n <= state["upper"], f"Population outside bounds: N={n}, {state}"
        if n >= state["upper"] and not state["growth_stopped"]:
            state["growth_stopped"] = True
            print(f"[CapReached] iter={iteration} N={n} bound=hard_upper growth_disabled_permanently=true")
        if n <= state["lower"] and not state["pruning_stopped"]:
            state["pruning_stopped"] = True
            print(f"[CapReached] iter={iteration} N={n} bound=hard_lower pruning_disabled_permanently=true")

    def freeze_population(self, iteration: int):
        self.begin_densification(iteration)
        state = self.densification_state
        if state["frozen_n"] is None:
            state["frozen_n"] = self.get_xyz.shape[0]
            print(f"[PopulationFrozen] iter={iteration} N_frozen={state['frozen_n']}")
        assert self.get_xyz.shape[0] == state["frozen_n"], "Population changed after densification window"
        self._check_population_bounds(iteration)
    ...
    def densify_and_prune(
        ...
        self.begin_densification(iteration)
        state = self.densification_state
        if state["frozen_n"] is not None:
            raise RuntimeError("Cannot densify or prune after the population is frozen")
        if n_splits < 2 or clone_scale_threshold > split_scale_threshold:
            raise ValueError("Require n_splits >= 2 and nonoverlapping clone/split scale ranges")
        self._check_population_bounds(iteration)
        n_before = self.get_xyz.shape[0]
        event_budget = n_before // 10
        grads = self.get_avg_xyz_grad()
        grads[torch.isnan(grads)] = 0.0

        # Select both branches from the same ORIGINAL rows/statistics. Clone's
        # postfix resets statistics and appends rows; neither may affect split.
        eligible = grads.squeeze(-1) >= max_grad
        if importance_threshold > 0.0:
            eligible &= self.get_avg_importance().squeeze(-1) >= importance_threshold
        pair_scale = self.get_pair_scaling().max(dim=1).values
        clone_mask = eligible & (pair_scale <= clone_scale_threshold)
        split_mask = eligible & (pair_scale > split_scale_threshold)
        clone_selected = int(clone_mask.sum().item())
        split_selected = int(split_mask.sum().item())
        growth_stopped = state["growth_stopped"]
        upper_room = state["upper"] - n_before
        growth_budget = 0 if growth_stopped else min(event_budget, upper_room)

        # nonzero returns ascending row indices. Take a prefix across BOTH
        # branches, with split charged only its net replacement cost.
        candidates = (clone_mask | split_mask).nonzero(as_tuple=True)[0]
        costs = torch.where(split_mask[candidates], n_splits - 1, 1)
        allowed = candidates[costs.cumsum(0) <= growth_budget]
        growth_mask = torch.zeros_like(eligible)
        growth_mask[allowed] = True
        clone_mask &= growth_mask
        split_mask &= growth_mask
        clone_applied = int(clone_mask.sum().item())
        split_applied = int(split_mask.sum().item())
        ...
        self.densify_and_clone(
            ...
            selected_pts_mask = clone_mask,
        )

        # Pad to the actual population; new rows cannot split in this event.
        added_rows = self.get_xyz.shape[0] - split_mask.shape[0]
        split_mask = torch.cat((split_mask, split_mask.new_zeros(added_rows)))
        ...
        n_after_growth = self.get_xyz.shape[0]
        assert n_after_growth == n_before + clone_applied + (n_splits - 1) * split_applied
        assert n_after_growth - n_before <= event_budget
        # Latch immediately, even if pruning moves N below the upper bound.
        self._check_population_bounds(iteration)

        def status(selected, applied, stopped, bound):
            if selected == 0:
                return "no_candidates"
            if stopped:
                return f"disabled_after_{bound}"
            return "truncated" if applied < selected else "applied"

        print(f"[Densify] iter={iteration} N_before={n_before} "
              f"clone_selected={clone_selected} clone_applied={clone_applied} "
              f"clone_status={status(clone_selected, clone_applied, growth_stopped, 'hard_upper')} "
              f"split_selected={split_selected} split_applied={split_applied} "
              f"split_status={status(split_selected, split_applied, growth_stopped, 'hard_upper')} "
              f"children_added={n_splits * split_applied} parents_replaced={split_applied} "
              f"N_after_growth={n_after_growth}")
        if clone_applied + split_applied < clone_selected + split_selected:
            bounds = ["hard_upper_latched"] if growth_stopped else [
                name for name, limit in (("event_growth_10pct", event_budget), ("hard_upper", upper_room))
                if limit == growth_budget
            ]
            print(f"[CapTruncate] iter={iteration} phase=growth N_before={n_before} "
                  f"selected={clone_selected + split_selected} applied={clone_applied + split_applied} "
                  f"N_after={n_after_growth} bound={'+'.join(bounds)}")
        ...
        prune_candidates = prune_mask.nonzero(as_tuple=True)[0]
        prune_selected = prune_candidates.numel()
        pruning_stopped = state["pruning_stopped"]
        lower_room = n_after_growth - state["lower"]
        prune_budget = 0 if pruning_stopped else min(event_budget, lower_room)
        prune_indices = prune_candidates[:prune_budget]
        prune_applied = prune_indices.numel()
        prune_mask = torch.zeros_like(prune_mask)
        prune_mask[prune_indices] = True
        if prune_applied:
            self.prune_points(prune_mask)
        n_after = self.get_xyz.shape[0]
        assert n_after == n_after_growth - prune_applied
        assert prune_applied <= event_budget
        self._check_population_bounds(iteration)
        print(f"[Prune] iter={iteration} N_before={n_before} N_before_prune={n_after_growth} "
              f"prune_selected={prune_selected} prune_applied={prune_applied} "
              f"prune_status={status(prune_selected, prune_applied, pruning_stopped, 'hard_lower')} "
              f"N_after={n_after}")
        if prune_applied < prune_selected:
            bounds = ["hard_lower_latched"] if pruning_stopped else [
                name for name, limit in (("event_shrink_10pct", event_budget), ("hard_lower", lower_room))
                if limit == prune_budget
            ]
            print(f"[CapTruncate] iter={iteration} phase=prune N_before={n_before} "
                  f"selected={prune_selected} applied={prune_applied} "
                  f"N_after={n_after} bound={'+'.join(bounds)}")
        # Also reset when neither growth branch selected a primitive.
        self._reset_statistics()
